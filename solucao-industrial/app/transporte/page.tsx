...
export default function TransportePage() {
  ...
  const [formData, setFormData] = useState<FormData>({
    descricao: '', valor: 0, mes: currentMonth, ano: selectedYear, observacao: '',
  });
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const data = await apiComplete.transporte.list(selectedYear);
      setRegistros(data);
    } catch (error: unknown) {
      console.error('Erro ao carregar dados:', error);
      const msg = error instanceof Error ? error.message : '';
      if (msg.includes('Usuário não autenticado')) return;
      alert('Erro ao carregar transporte');
    } finally {
      setLoading(false);
    }
  ...
  const handleCreate = () => {
    setFormData({ descricao: '', valor: 0, mes: currentMonth, ano: selectedYear, observacao: '' });
    setFormErrors({});
    setIsModalOpen(true);
  };
  ...
  return (
    <MainLayout title="Transporte">
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-4">
            <p className="text-gray-600">Custos de transporte mensal</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-medium text-gray-600">Total Transporte - {selectedYear}</p>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 overflow-x-auto">
            <p className="text-sm font-medium text-gray-600 mb-2">Resumo Mensal</p>
            <div className="grid grid-cols-4 gap-1 text-xs">
              {resumoMensal.map((m, i) => (
                <div key={i} className="text-center">
                  <span className="text-gray-500 block">{m.mes.substring(0, 3)}</span>
                  <span className={`font-medium ${m.total > 0 ? 'text-gray-900' : 'text-gray-300'}`}>
      ...
      {loading ? (
        ...
        <DataTable data={registros} columns={columns} keyExtractor={(r) => r.id} searchKeys={['descricao']} emptyMessage="Nenhum registro de transporte" />
      )}

      <FormModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSubmit={handleSubmit} title="Novo Custo de Transporte" submitText="Criar" isLoading={submitLoading}>
        <div className="space-y-4">
          <div>
            ...
            <input type="text" value={formData.descricao} onChange={(e) => setFormData({ ...formData, descricao: e.target.value })}
              ...
              placeholder="Ex: Frete para cliente X" />
